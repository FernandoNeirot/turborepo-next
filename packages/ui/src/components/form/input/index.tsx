...
const Input = ({ 
  ...
  return (
    <div className="flex flex-col gap-1 w-full">
      {label && (
        <label className={`text-sm font-medium ${isDisabled ? 'text-gray-300' : 'text-gray-700'}`}>
          {label}
        </label>
      )}      
      <input
      ...
      {helperText && !error && (
        <span className={`text-sm ${isDisabled ? 'text-gray-200' : 'text-gray-500'}`}>{helperText}</span>
      )}
    </div>
  )
}
